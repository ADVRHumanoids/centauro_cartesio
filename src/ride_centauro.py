...
robot.setStiffness({"j_arm1_4": 100})
robot.move()
...
zero_pose_l = robot.model().getPose('arm1_8')
print zero_pose
...
start_pose_pelvis,_,_ = pelvis.getPoseReference()
squat_down = start_pose_pelvis.copy()
squat_down.translation[2] -= 0.1
...
while not rospy.is_shutdown():
    ...
    # pelvis
    cart_error_z_l = zero_pose_l.translation[2] - robot.model().getPose('arm1_8').translation[2]
    #print cart_error_z_l

    if cart_error_z_l > 0.05 and pelvis.getTaskState() == pyci.State.Online :
        pelvis.setPoseTarget(squat_down, 2)
    
    if cart_error_z_l < -0.03 and pelvis.getTaskState() == pyci.State.Online :
        pelvis.setPoseTarget(start_pose_pelvis, 2)
